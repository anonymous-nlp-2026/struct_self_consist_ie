#!/usr/bin/env python3
"""5-signal + DGS analysis for LLaMA SciERC seed=123."""
import json, sys, os
import numpy as np
...
# Greedy F1 for conditional split
greedy_f1s_all = []
for inst in valid:
...
# === 5-Signal Analysis ===
print("\n=== 5-Signal Analysis ===")
report = {"model": "LLaMA-3.1-8B-Instruct", "dataset": "SciERC", "seed": 123,
...
for split_name, split_insts in [("full", valid), ("conditional", conditional)]:
    ...
    for inst in split_insts:
        ...
        sample_f1_lists.append([compute_entity_f1(s.get("entities", []), inst["gold"]["entities"]) for s in samples])
    ...
    split_res = {
        "n": len(split_insts),
        "greedy_f1_mean": round(float(f1_arr.mean()), 4),
        "oracle_f1_mean": round(float(oracle_arr.mean()), 4),
        "headroom": round(float(oracle_arr.mean() - f1_arr.mean()), 4),
        "pct_perfect": round(float((binary==1).mean()) * 100, 2),
    }
