#!/usr/bin/env python3
"""exp_017: 5-signal evaluation for LLaMA CoNLL-2003 NER inference."""
...
def main():
    ...
    # Conditional: also filter greedy-F1=0
    greedy_f1s_all = []
    for inst in valid:
    ...
    for split_name, split_instances in [("full", valid), ("conditional", conditional)]:
        ...
        for sig_name, sig_vals in signals.items():
            ...
            metrics[sig_name] = m
            print(f"  {sig_name:>12}: rho={rho:.4f}  AUROC={auroc:.4f}  tau={tau:.4f}  ECE={m['ECE']['value']:.4f}")
