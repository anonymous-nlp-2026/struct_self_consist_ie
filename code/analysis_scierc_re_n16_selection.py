#!/usr/bin/env python3
"""SciERC N=16 3-seed RE Selection F1 analysis."""
...
from consistency import _re_soft_jaccard_pair, _extract_surface_keys
...
def compute_sample_sj_scores(instance, subtask):
    ...
    for i in range(N):
        for j in range(i + 1, N):
            s = _re_soft_jaccard_pair(samples[i].get(field, []), samples[j].get(field, []))
            matrix[i][j] = s
            matrix[j][i] = s
...
def main():
    ...
    # Save RE-only results
    out_path = f"{BASE}/output/analysis_scierc_re_n16_3seed_selection_f1.json"
    with open(out_path, "w") as f:
        json.dump(all_results, f, indent=2)
    print(f"\nSaved RE results: {out_path}")

    # Merge into existing NER JSON
    ner_path = f"{BASE}/output/analysis_scierc_n16_3seed_selection_f1.json"
    if os.path.exists(ner_path):
        with open(ner_path) as f:
            merged = json.load(f)
        merged.update(all_results)
        merged_path = f"{BASE}/output/analysis_scierc_n16_3seed_selection_f1_merged.json"
        with open(merged_path, "w") as f:
            json.dump(merged, f, indent=2)
        print(f"Saved merged (NER+RE): {merged_path}")
