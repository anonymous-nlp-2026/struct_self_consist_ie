#!/usr/bin/env python3
"""5-signal analysis for exp-026 temperature ablation (parameterized)."""
...
def compute_degeneracy(data, n_samples=8):
    degenerate = 0
    total = 0
    for inst in data:
...
def compute_lp_range(data, n_samples=8):
    ranges = []
    for inst in data:
        samples = inst.get("samples", [])[:n_samples]
        if len(samples) < n_samples:
    ...
    return float(np.mean(ranges)), float(np.std(ranges)), len(ranges)
...
def evaluate_subtask(instances, subtask):
    ...
    for split_name, split_instances in [("full", valid), ("conditional", conditional)]:
        ...
        for sig_name, sig_vals in signals.items():
            ...
            m["ECE"] = {"value": compute_ece(ece_conf, binary_correct)}

            metrics[sig_name] = m
            print(f"  {sig_name:>12}: rho={rho:.4f}  tau={tau:.4f}  AUROC={auroc:.4f}  ECE={m['ECE']['value']:.4f}")
...
def main():
    ...
    parser.add_argument("--subtasks", default="ner,re", help="Comma-separated subtasks")
    args = parser.parse_args()

    subtasks = [s.strip() for s in args.subtasks.split(",")]
    instances = load_data(args.data_path)
    print(f"Loaded {len(instances)} instances, N={len(instances[0]['samples'])}")

    # Degeneracy and LP range
    deg_rate, deg_count, deg_total = compute_degeneracy(instances)
    lp_mean, lp_std, lp_n = compute_lp_range(instances)
    print(f"\nDegeneracy rate: {deg_rate:.4f} ({deg_count}/{deg_total})")
    print(f"LP range: mean={lp_mean:.6f}, std={lp_std:.6f}, n={lp_n}")

    # 5-signal per subtask
    all_results = {}
    for subtask in subtasks:
        all_results[subtask] = evaluate_subtask(instances, subtask)

    all_results["degeneracy"] = {"rate": deg_rate, "count": deg_count, "total": deg_total}
    all_results["lp_range"] = {"mean": lp_mean, "std": lp_std, "n": lp_n}

    import os
    os.makedirs(args.output_dir, exist_ok=True)
    out_path = os.path.join(args.output_dir, "all_signals_5signal.json")
