...
def analyze_subtask(instances, subtask):
    ...
    # Conditional set: exclude instances where all N sample F1s = 0
    cond_valid = []
    for i in valid:
        inst = instances[i]
        sample_f1s = [per_instance_f1(s, inst["gold"], subtask) for s in inst["samples"]]
        if any(f > 0 for f in sample_f1s):
            cond_valid.append(i)
