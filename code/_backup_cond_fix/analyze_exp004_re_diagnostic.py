...
def threshold_sensitivity(data, thresholds=None):
    ...
    for thr in thresholds:
        ...
        if thr == 0:
            subset = [x for x in instances if x["max_sample_f1"] > 0]
        else:
...
def compute_bootstrap_cis(data):
    ...
    # Conditional RE (exclude all-sample-f1=0)
    re_cond = []
    for inst in re_insts:
        sf1s = [instance_f1(s, inst["gold"], "re") for s in inst["samples"]]
        if any(f > 0 for f in sf1s):
            re_cond.append(inst)
