...
def main():
    ...
    # Conditional mask: at least one sample has F1 > 0
    cond_mask = []
    for inst in re_instances:
        any_pos = any(per_instance_f1(s, inst['gold'], subtask='re') > 0 for s in inst['samples'])
        cond_mask.append(any_pos)
