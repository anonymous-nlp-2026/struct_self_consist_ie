#!/usr/bin/env python3
"""Compute 5-signal all_signals_report.json for exp_001 3 seeds (NER, Qwen/SciERC)."""
...
SEEDS = {
    "seed42": "./output/exp_001_seed42_v2",
    "seed123": "./output/exp_001_seed123_v2",
    "seed456": "./output/exp_001_seed456_v2_ner",
}
SUBTASK = "ner"
...
def main():
    ...
    for seed_name, out_dir in SEEDS.items():
        print(f"\n{'='*60}")
        print(f"Processing {seed_name}: {out_dir}")
        ...
        # Save per-seed all_signals_report.json
        out_path = os.path.join(out_dir, "all_signals_report.json")
        with open(out_path, "w") as f:
            json.dump(seed_result, f, indent=2, default=json_default)
        print(f"\n  Saved: {out_path}")
        all_seed_results[seed_name] = seed_result

    # Print summary JSON for all seeds
    print(f"\n{'='*60}")
    print("ALL SEEDS COMBINED:")
    print(json.dumps(all_seed_results, indent=2, default=json_default))
