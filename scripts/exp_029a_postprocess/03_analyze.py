...
6. Comparison table vs ft_002_v2 (5-epoch baseline)
...
# ft_002_v2 reference (SciERC 5-epoch baseline)
BASELINE_REF = {
    "degeneracy_cf1_pct": 17.8,
    "lp_sel_delta_pp": 0.42,
    "within_instance_median_rho": 0.6667,
}
...
def main():
    print("=" * 70)
    print("EXP_029a: SciERC 3-epoch Degeneracy Analysis")
    print("=" * 70)
    ...
    for sig_name in ["SJ", "FK", "EM", "VC"]:
        if len(all_signal_scores[sig_name]) != len(all_f1_scores):
            pass
    ...
    # === 6. Comparison Table ===
    print("\n--- 6. Comparison: 3-epoch vs 5-epoch (SciERC) ---")
    print(f"  {'Metric':<25} | {'5-epoch (ft_002_v2)':>18} | {'3-epoch (exp_029a)':>18} | {'Delta':>8}")
    print(f"  {'-'*25}-+-{'-'*18}-+-{'-'*18}-+-{'-'*8}")
    ...
    rows = [
        ...
        ("Greedy F1", "N/A", f"{greedy_macro_f1:.4f}", ""),
        ...
        ("Within-inst median rho", f"{BASELINE_REF['within_instance_median_rho']:.4f}",
    ...
    results = {
        "experiment": "exp_029a",
        "description": "SciERC 3-epoch LoRA degeneracy test",
        "n_total": n_total,
