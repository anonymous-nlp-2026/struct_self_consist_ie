#!/usr/bin/env python3
"""E_theta Variant 3: Without dataset_id (Pure Instance Features) + Ridge Regression
Tests cross-dataset generalization by removing dataset_id features."""
...
def process_instance(inst, config):
    ...
    features = np.array([
        ...
        lp_var,
        float(N),
        float(config["model_id"] == 0),    # is_qwen
        float(config["model_id"] == 1),    # is_llama
        agr_mean,
        float(len(all_ents)),
    ])
...
def main():
    print("=" * 60)
    print("E_theta V3: No dataset_id + Ridge Regression")
    print("=" * 60)
    ...
    for config in CONFIGS:
        ...
        if not path.exists():
            print(f"SKIP {config['name']}: file not found")
            continue

        print(f"\nProcessing {config['name']} ...", flush=True)
        results = []
        with open(path) as f:
    ...
    if v1_path.exists():
        ...
        v1_fix = v1["overall_fixed_f1"]
        v1_delta = v1["delta_adaptive_vs_fixed"]
        v3_delta = oa - of_
        v1_comparison = {
            "v1_overall_adaptive_f1": v1_adap,
            "v1_delta_adaptive_vs_fixed": v1_delta,
            "v3_delta_adaptive_vs_fixed": round(v3_delta, 6),
            "gap_v3_minus_v1_delta": round(v3_delta - v1_delta, 6),
            "gap_v3_minus_v1_adaptive_f1": round(oa - v1_adap, 6),
            "dataset_id_matters": abs(oa - v1_adap) > 0.01,
        }

    output = {
        "variant": "v3_no_dataset_id_ridge",
        "description": "No dataset_id features (is_scierc, is_conll, is_fewnerd removed). Tests cross-dataset generalization.",
    ...
    out_path = BASE / "output" / "e_theta_v3_no_dataset_results.json"
    ...
    if v1_comparison:
        ...
        print(f"  v3 adaptive F1: {oa:.6f}")
        print(f"  gap (v3-v1):    {v1_comparison['gap_v3_minus_v1_adaptive_f1']:+.6f}")
        print(f"  dataset_id matters: {v1_comparison['dataset_id_matters']}")
    print(f"\nSaved to {out_path}")
